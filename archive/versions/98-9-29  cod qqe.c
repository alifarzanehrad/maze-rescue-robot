    ...
    while(T0<STR)
        ...
        if (SDR>=400 && SDL>=400)
            ...
            while(T0<savet0)
                {
                check_error();
                if (T0<savet0*0.15 || T0>savet0*0.85)
...
void  menu(void)
    {
    static unsigned char page=1,last_page=7;
    float battery;
    lcd_init(20);
    lcd_clear();
...
while(1)
        ...
        sprintf(text,"add=%3d   temp=%3d  ",add,MLX(add));
        lcd_gotoxy(0,0);
        lcd_puts(text); 
        delay_ms(100);
        if (next_page==0)
       ...
       if(SDL<100 && SDR<100) // check piont
    {
        ...
        //finish:
       {
            ...
            if (((x==xs+2 || x==xs || x==xs-2|| x==xs-1 || x==xs+1 )&&( y==ys+2 || y==ys ||y==ys-2 ||y==ys+1 ||y==ys-1 ))|| flagt>=3)  // finish
                {
                 if(flagt>=3 || ((x==xs+2 || x==xs-2 )&&( y==ys+2 || y==ys-2 )) || Olaviat==3)//Olaviat==3 Chon Zamin 2 ta Ramp Dre!
                 {
            ...
            else if((Olaviat==3  )|| flagt>=3)//&& returning==1
                {
                 lcd_clear();
                 //lcd_putchar(0xFF);
                 //delay_ms(4000); 
