...
void DirCheck(void)
{
 ...
 //if(hasGoneUp)dir=4;
// sprintf (text,"deg=%d N=%d E=%d S=%d W=%d",deg,djN,djE,djS,djW);
// lcd_gotoxy(0,0);
// lcd_puts(text);
// delay_ms(5000);
}
...
void  menu_sensor(void)
    ...
    ///////////////
        ...
        if(page==2)    // ultra
            ...
            {
           // lcd_clear();
            sprintf (text,"CHARKH=%02f",CHARKH);
            lcd_gotoxy(0,0);
            lcd_puts(text);
            
            if (enter==0)
            {
            while(KFL+KFR !=0 || UF<90)
              {
              go(180,180);
...
void  menu_pist_calib (void)
    ...
    sprintf(text,"       DN=%3d    ",djN);
    lcd_gotoxy(0,2);
    lcd_puts(text);

    sprintf(text,"DW=%3d DS=%3d DE=%3d",djW,djS,djE);
    lcd_gotoxy(0,3);
    lcd_puts(text);
