...
void check_error(void)  
    {   
    if (sharpL>120  && abs(ULF-ULB)<100 && ULF<200 && ULB<200 )
        { 
        error1=(ULF-ULB)*3;
        error2=ULF-70;  
        error=error1+error2; 
    ...
    else if (sharpR>120 && abs(URF-URB)<100 && URF<200 && URB<200 )
        {
...
//            }
    ...
    if (SDR>=400 && SDL>=400)//black room
            ...
            while(T0<STR+100)
                {
                check_error();
                if (T0<STR*0.15 || T0>STR*0.85)
                    go((-1)*(70+error),(-1)*(70+error));
                else
                    go((-1)*(100+error),(-1)*(100+error));
                }
                go(0,0);
                calculate();
            if (dir == 1)
...
void  menu_sensor(void)
    ...
    ///////////////
        ...
        if(page==2)    // ultra
            ...
            {
            sprintf(text,"ur=%2d  ul=%2d    ",sharpR,sharpL);
            lcd_gotoxy(0,0);
            lcd_puts(text);
            delay_ms(50);
